...
First we will look at the likelihood function for our data, which has a single binary response and `r 30-length(bad)` numerical explanatory variables. We know that
\[
...
where $\mathbf{X_i}$ represents the $i$-th observation of all 20 of our predictor variables. 
For our data, the likelihood function is given by 
\[ L(\mathbf{X}| \mathbf{\beta}) = \prod^n_{i=1} \left[ \pi_i^{y_i}(1-\pi_i)^{1-y_i} \right]
\]
where $y_i$ represents out binary outcome for the $i$-th individual. 
