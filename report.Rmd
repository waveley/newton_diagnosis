...
Breast cancer has become one of the most common kinds of cancer in the United States, and substantial efforts have been made to aid in early and accurate detection. Along with national public health initiatives encouraging women at certain ages to schedule mammograms, great strides have been made to improve tumor imaging technology used in screening procedures. These improvements allow clinicians more pathways to early diagnosis, as we have access to more data than ever before. However, data is not necessarily in bijective correspondence with _information_ [1]. With more data comes more noise, and it becomes more important for statisticians and medical practitioners to separate signal from that noise.

## 1.2. Objectives

We investigate two key questions: (1) Does having more data always correspond to an advantage in diagnosis prediction? (2) Can we reduce the amount of data we need to collect while maintaining (or increasing) predictive power? Toward that end, we use a breast cancer imaging dataset (detailed below) to fit two different models, both of which aim to predict patient diagnosis outcomes.

\vspace{5mm}

Our first model is a logistic regression model with a logit link consisting of a full set of predictors (i.e., the "full model") and model parameters estimated through a Newton-Raphson optimization algorithm. The second model is a logistic LASSO model that (possibly) selects fewer predictors (i.e., the "optimal model"), which is implemented using a path-wise coordinate descent optimization algorithm and tuned via a five-fold cross validation to obtain an optimal penalization term ($\lambda$). The algorithms for each method are discussed below in the methods section and corresponding code can be found in the appendix.

# 2. Methods

## 2.1. Data Cleaning and Exploratory Analysis

The dataset of interest contains 569 rows and 32 columns related to breast tissue imaging with each observation representing an individual patient. The outcome of interest is patient diagnosis, taking on values of either malignant or benign. One column contains information about patient ID, which will be excluded from our analysis. The other 30 columns correspond to summary statistics (mean, standard error, and maximum) of tumor radius, texture, perimeter, area, smoothness, compactness, concavity, concave points, symmetry, and fractal dimension. This dataset does not contain any missing values.
...
In exploring the data, we find many of the predictors are highly correlated with one another. In Figure 1, we see a correlation plot where variables with high correlations are denoted by dark shading. High collinearity between predictors is a threat to the convergence of our algorithms, particularly in the context of high-dimensional data. To explore the correlation further, Figure 2 shows the 25 largest correlations in our data. We see that the highest correlation is between `radius_mean` and `perimeter_mean` with a correlation coefficient of 0.998. In our dataset, there are 21 combinations of variables that achieve a correlation exceeding 0.90, which is a cause for concern in this context. We can break these 21 pairings into quasi-equivalence classes\footnote{We do not have precise equivalence classes, as transitivity only holds approximately. In particular, for variables $x_1, x_2, x_3$, $\text{corr}(x_1, x_2) > 0.90$ and $\text{corr}(x_2, x_3) > 0.90$ does not guarantee that $\text{corr}(x_1, x_3) > 0.90$, but $x_1$ and $x_3$ will still be highly correlated.} for further inspection, where each quasi-equivalence class consists of variables that are all mutually pairwise correlated.
...
The first grouping of mutually correlated variables is {`area_mean`, `area_worst`, `perimeter_mean`, `perimeter_worst`, `radius_mean`, `radius_worst`}. This grouping represents 15 of the pairs in Figure 2. While we want to reduce the problem of collinearity, we do not want to simply discard all of these variables. Instead, we seek to identify the member of this class that best correlates with all other members. To identify the best representative for this class, we look at the variable with the highest mean correlation. In this case, we select `radius_worst`.

\vspace{5mm}

We repeat this procedure for each other quasi-equivalence class. From the class {`radius_se`, `perimeter_se`, `area_se`}, we select `radius_se`. From the class {`concavity_mean`, `concave_points_worst`, `concave_points_mean`}, we select `concave_points_mean`. Finally, for class {`texture_mean`, `texture_worst`}, the choice of representative is arbitrary, se we simply select `texture_mean`. Given our best representatives in each class, we discard the remaining predictors, leaving `r 30 - length(bad)` predictors in our dataset. All surviving predictors can be seen in Table 1.
...
In Table 1, we see that there are 357 benign (B) cases and 212 malignant (M) cases. To evaluate the performance of both models, we partition our data into a training set (80\%) and a testing set (20\%). The data is standardized before fitting both of our models, which improves run times and simplifies parameter interpretation. For LASSO regression in particular, it is best practice to center and scale data, as the $\ell_1$-norm penalization in LASSO regression tends to unequally penalize coefficient estimates if the covariates are of different magnitudes or scales. Therefore, standardizing our data ensures equal weighting and penalization.

\vspace{5mm}

\begin{center}
{\bf Table 1: Patient Characteristics}
\end{center}
...
Remarkable! The above result informs us that maximizer of $g$ (and hence, the approximate maximizer of $l$) subject to our LASSO constraint is simply the un-penalized maximizer $\hat{\beta}_k$ shrunken by weight-adjusted penalty $\lambda_k$. Given this, our coordinate descent algorithm proceeds as follows.

- Outer Loop: Decrement over $\lambda \in (\lambda_{\max}, \ldots, \lambda_{\min})$, where $\lambda_{\max} \equiv \frac{1}{n} \max \{|\mathbf{X}^t \mathbf{Y}|\}$.
...
Note: the middle loop terminates when a given Taylor expansion no longer yields updates (within the specified tolerance) to $\boldsymbol\beta$ in the inner loop. That is, if we update $\boldsymbol\alpha$ (and hence, $\pi_i, w_i, z_i,$ and $g$) in the middle loop, and if we then find that no elements of $\boldsymbol\beta$ are updated in the following inner loop, we have found our optimal $\boldsymbol\beta$ given penalty $\lambda$.



## 2.4 Five-fold Cross Validation 

As the performance of the logistic LASSO model depends on the penalty factor ($\lambda$), we perform cross validation in order to select the $\lambda$ that produces a model that optimizes a metric of interest.

\vspace{5mm}

First, we define our range of possible $\lambda$ values, where the largest value produces a model with no predictors selected and the smallest value would produce a model with all predictors selected. We define the maximum value as the smallest penalty for which $\beta_k = 0$ for all $k\in\{1, \dots, p\}$. This value works out to be the maximum of the inner product between our predictors ($X$) and our outcome ($Y$) from the imaging dataset. For $\lambda_{min}$, we select the a sufficiently small value that selects the full model. As recommended by Friedman et. al (2009) [3], we started by setting the smallest value in our range to be $\lambda_{min} = \frac{\lambda_{max}}{1000}$. However, we were not able to use this value as as the minimum $\lambda$ in our range because it did not select the full model. Thus, we used the suggested $\frac{\lambda_{max}}{1000}$ as a starting point and empirically derived our $\lambda_{min}$ by halving the working value until the full model was selected. Ultimately, we find that $0.0001\,\,\left(\approx\frac{\lambda_{max}}{4000}\right)$ is sufficiently small to select the full model. We then constructed a sequence between $\log{\lambda_{max}}$ and $\log{\lambda_{min}}$ so that the sequence would contain 100 values with equal intervals between values and exponentiated all values in that sequence. In this fashion, we thus created a range of 100 values between $\lambda_{max}$ and $\lambda_{min}$ on a logarithmic scale. 
...
\begin{center}
{\bf Figure 3: Cross Validation Procedure}
\end{center}
...
When evaluating our cross validation results, we first confirm that our range of $\lambda$ values has one maximum AUC value when using the greatest mean AUC optimal lambda method. This can be seen in Figure 4, where the largest mean AUC and corresponding lambda value has been identified.

\vspace{5mm}

\begin{center}
{\bf Figure 4: Cross Validation Results: Selecting Best Lambda}
\end{center}
...
We can also evaluate our cross validation results by looking at the beta estimates for each corresponding $\lambda$ value (Figure 5). The smallest value of -log(lambda) corresponds with our null model where all estimates of $\beta_j$ are zero and the $\lambda$ penalization term is large. The largest value of -log(lambda) corresponds with a full model where no $\beta_j$ values are zero. We see that the greatest mean AUC selects a larger $\lambda$ value, greater penalization, and thus a model with fewer predictor coefficients compared to the minimax AUC method. 

\newpage

\begin{center}
{\bf Figure 5: Cross Validation Results: LASSO Coefficients}
\end{center}
...
We then compared the $\beta_i$ estimates in each of our models (Table 2). As our data had been standardized prior to constructing our models, these estimates must be interpreted with caution (especially so for the logistic LASSO model, with its inclusion of a penalty term). We found that the logistic LASSO model constructed using a $\lambda$ that maximized greatest mean AUC has the most $\beta_i$ values of 0, indicating that this penalty term selects the fewest predictors. The logistic LASSO model constructed using a $\lambda$ that optimized on Minimax AUC selects 16 nonzero $\beta_i$ values. Of importance to note is that between the three models, the smaller models are always nested within the larger models -- that is, all the predictors contained in the smallest logistic LASSO model are contained within the larger logistic LASSO model, and all of the predictors contained within the larger logistic LASSO model are contained within the full logistic linear model. 

\vspace{5mm}

We then compared the $\beta_i$ estimates in each of our models (Table 2). As our data had been standardized prior to constructing our models, these estimates must be interpreted with caution (especially so for the logistic LASSO model, with its inclusion of a penalty term). We found that the logistic LASSO model constructed using a $\lambda$ that maximized greatest mean AUC has the most $\beta_i$ values of 0, indicating that this penalty term selects the fewest . Table 3 also confirms that greatest mean AUC produces a model with 6 nonzero beta values with lambda value 0.0101, not including the intercept term. Minimax optimization selects 16 nonzero beta values with lambda value 0.0006. While each model has test AUC values above 0.97, the model with the largest test AUC is the greatest mean optimal model.

\newpage

\begin{center}
{\bf Table 2: Beta Coefficients Comparing Full and Optimal Models}
\end{center}
...
We then compared the two measures of model fit for these three models. When looking at the prediction AUC produced by each of these models on the test set we had withheld, we found that the smaller logistic LASSO model produced an AUC of 0.9897, the larger logistic LASSO model produced an AUC of 0.9858, and the full logistic linear model produced an AUC of 0.9765. Thus, we found that the logistic LASSO models both out-performed the logistic linear model, with the smallest logistic LASSO model producing the largest AUC overall. 

\vspace{5mm}

While the AUC value is a good indicator of overall classification performance, we are still confronted with the trade-off between sensitivity and specificity (Figure 6). Imagining is typically the first line of defense for diagnosing cancers, so optimizing sensitivity is usually of great interest. Thus, ideally, for clinical diagnosis purposes, it would be good to recommend a model that sacrifices the least amount of specificity while still achieving a sensitivity of 1 (Table 3). We found that the greatest mean AUC lasso model has the highest specificity of 69.23% when holding sensitivity at 1. Though the larger logistic LASSO model had its penalty term selected using the "minimum-worst-AUC" criteria in our cross validation process, it still achieved a much larger specificity than the full model, 55.38% when controlling for sensitivity. Namely, if we choose to a lasso model, it is still expected to manage to achieve 55.38% specificity while maintaining a sensitivity of 1 in the worst case scenario. It is to be observed that full model has the lowest sensitivity, 24.62%, and hence needs to sacrifice specificity more than 75% to achieve 1 sensitivity.

\newpage

\begin{center}
{\bf Table 3: Model Summary Comparing Full and Optimal Models}
\end{center}
...
\begin{center}
{\bf Figure 6: Comparing Full and Optimal Models Performance}
\end{center}
...
Lastly, we find that that 6 variables in particular - `texture_mean`, `concave_points_mean`, `radius_worst`, `smoothness_worst`, `concavity_worst`, and `symmetry_worst` - play important roles in predicting whether breast imaging data is malignant.

## 4.2. Limitations

While the initial reduction of variables to limit high correlations was beneficial, we may have not selected the best representative of the correlation group. We did not try different representatives of the equivalence classes and so this might be a limitation that could prompt further study. Additionally, though we primarily used AUC as a measure of model performance in our study, future studies may find the measure of the accuracy rate of prediction interesting to use, if the information of the cutoff probability for classification is given from medical professionals.
...
\begin{enumerate}[label={[\arabic*]}]
  ...
  \item  Jerome Friedman. Trevor Hastie. Holger Höfling. Robert Tibshirani. "Pathwise coordinate optimization." Ann. Appl. Stat. 1 (2) 302 - 332, December 2007. https://doi.org/10.1214/07-AOAS131.
...
In this appendix entry, we show that the log-likelihood function $f$ can be approximated via the quadratic function $g : \mathbb{R}^p \to \mathbb{R}$ given by
	\[ g(\boldsymbol\beta) = - \frac{1}{2} \sum_{i = 1}^n w_i \left(z_i - \sum_{j = 1}^p \beta_j x_{ij} \right)^2 + \eta(\boldsymbol\alpha). \]
Fix vectors $\boldsymbol\beta \in \mathbb{R}^p$. For each observation $i \in \{ 1, \ldots, n \}$, we first define
	\[ \xi_i \equiv \sum_{j = 1}^p x_{ij} (\beta_j - \alpha_j), \quad \text{such that} \]
	\begin{align*}
	 	\frac{y_i - \pi_i}{w_i} - \xi_i
		&= \frac{y_i - \pi_i}{w_i} - \sum_{j = 1}^p x_{ij} (\beta_j - \alpha_j)\\
		&= \left( \frac{y_i - \pi_i}{w_i} + \sum_{j = 1}^p x_{ij} \alpha_j\right) - \sum_{j = 1}^p x_{ij} \beta_j\\
		&= z_i - \sum_{j = 1}^p x_{ij} \beta_j.
	\end{align*}
We proceed with a series of computations. Observe that
	\begin{align*}
		\Big(\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)\Big)^t (\mathbf{Y} - \boldsymbol\pi)
		&= \left( \begin{pmatrix} x_{11} & \cdots & x_{1p} \\ \vdots & \ddots & \vdots \\ x_{n1} & \cdots & x_{np} \end{pmatrix}
			\begin{pmatrix} \beta_1 - \alpha_1 \\ \vdots \\ \beta_p - \alpha_p \end{pmatrix} \right)^t
			\begin{pmatrix} y_1 - \pi_1 \\ \vdots \\ y_n - \pi_n \end{pmatrix} \\
		&= \begin{pmatrix} \sum_{j = 1}^p x_{1j} (\beta_j - \alpha_j) \\ \vdots \\ \sum_{j = 1}^p x_{nj} (\beta_j - \alpha_j) \end{pmatrix}^t
			\begin{pmatrix} y_1 - \pi_1 \\ \vdots \\ y_n - \pi_n \end{pmatrix} \\
		&= \sum_{i = 1}^n \sum_{j = 1}^p x_{ij} (\beta_j - \alpha_j)(y_i - \pi_i) \\
		&= \sum_{i = 1}^n (y_i - \pi_i) \xi_i.
	\end{align*}
Because $W$ is a diagonal matrix with non-negative elements, $\sqrt{\mathbf{W}}$ is well defined. As such, we can compute
	\begin{align*}
		\sqrt{\mathbf{W}}\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)
		&= \begin{pmatrix} \sqrt{w_1} & \cdots & 0 \\ \vdots & \ddots & \vdots \\ 0 & \cdots & \sqrt{w_n} \end{pmatrix}
			\begin{pmatrix} x_{11} & \cdots & x_{1p} \\ \vdots & \ddots & \vdots \\ x_{n1} & \cdots & x_{np} \end{pmatrix}
			\begin{pmatrix} \beta_1 - \alpha_1 \\ \vdots \\ \beta_p - \alpha_p \end{pmatrix} \\
		&= \begin{pmatrix}  \sqrt{w_1} x_{11} & \cdots &  \sqrt{w_1} x_{1p} \\ \vdots & \ddots & \vdots \\ \sqrt{w_n} x_{n1} & \cdots & \sqrt{w_n} x_{np} \end{pmatrix}
			\begin{pmatrix} \beta_1 - \alpha_1 \\ \vdots \\ \beta_p - \alpha_p \end{pmatrix} \\
		&= \begin{pmatrix} \sum_{j = 1}^p \sqrt{w_1} x_{1j} (\beta_j - \alpha_j)  \\ \vdots \\ \sum_{j = 1}^p \sqrt{w_n} x_{1n} (\beta_j - \alpha_j) \end{pmatrix}.
	\end{align*}
Given the above computation, we further obtain
	\begin{align*}
		\Big(\sqrt{\mathbf{W}}\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)\Big)^t \Big(\sqrt{\mathbf{W}}\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)\Big)
		&= \sum_{i = 1}^n \left( \sum_{j = 1}^p \sqrt{w_i} x_{ij} (\beta_j - \alpha_j) \right)^2 \\
...
Finally, we consider the second-order Taylor expansion of $f$ centered at $\boldsymbol\alpha$. In particular, given all of the above computations, and by completing the square, we obtain
	\begin{align*}
		f(\boldsymbol\beta)
		&\approx f(\boldsymbol\alpha) +
			(\boldsymbol\beta - \boldsymbol\alpha)^t \nabla f(\boldsymbol\alpha) +
			\frac{1}{2} (\boldsymbol\beta - \boldsymbol\alpha)^t \nabla^2 f(\boldsymbol\alpha) (\boldsymbol\beta - \boldsymbol\alpha) \\
		&= f(\boldsymbol\alpha) +
			(\boldsymbol\beta - \boldsymbol\alpha)^t \Big( \mathbf{X}^t (\mathbf{Y} - \boldsymbol\pi) \Big) +
			\frac{1}{2} (\boldsymbol\beta - \boldsymbol\alpha)^t \Big(-\mathbf{X}^t \mathbf{W} \mathbf{X} \Big) (\boldsymbol\beta - \boldsymbol\alpha) \\
		&= f(\boldsymbol\alpha) +
			\Big(\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)\Big)^t (\mathbf{Y} - \boldsymbol\pi) -
			\frac{1}{2} \Big(\sqrt{\mathbf{W}}\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)\Big)^t \Big(\sqrt{\mathbf{W}}\mathbf{X}(\boldsymbol\beta - \boldsymbol\alpha)\Big) \\
	...
	\end{align*}
where $\eta : \mathbb{R}^p \to \mathbb{R}$ is some function of $\boldsymbol\alpha$ that does not depend on $\boldsymbol\beta$.