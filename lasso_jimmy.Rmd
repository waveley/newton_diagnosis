...
q    <- 30 - 1
n    <- 1000
X    <- scale(matrix(rnorm(q * n), c(n, q)))
...
results <- tibble(
  ...
  , Difference = abs(Jimmy - GLM)
  , Change     = (Jimmy - GLM) / GLM
) %>%
  filter(Jimmy != 0 | GLMNET != 0)

results %>% knitr::kable()

c(mean(results$Difference[-1]), mean(results$Change[-1]), 1/n)
```
...
for(lambda in c(10, 1, 0.1, 0.01, 0.001)) {
# (max(t(X) %*% y) / n)
